...
SELECT `title`,`genre`,`release_year` AS  "Release" FROM `movies` WHERE `release_year` BETWEEN 2008 #AND 2015;
SELECT  `title`,`genre`,`release_year`,`rating` > 8.5 AND `rating` AS "High Rated Movies" FROM #`movies`;